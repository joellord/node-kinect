...
**/
var BaseGesture = function(skeleton, _options) {
...
BaseGesture.prototype.startTracking = function() {
	if (DEBUG) {console.log("startTracking");}
	...
	var trackingFn = function() {
		...
		if (self.finalConditionMet()) {
			if (DEBUG) {console.log("detected final condition met");}
			self.startDetection();
			...
			if (DEBUG) {console.log("gesture was not completed");}
			self.resetTrackingParams();
...
BaseGesture.prototype.startDetection = function() {
	...
	self.detectorTimer = setInterval(function() {detect.call(self);}, params.detectionRate);
}
...
BaseGesture.prototype.pauseDetection = function(ms) {
	var self = this;
	this.stopDetection();
	setTimeout(function() {self.startDetection.call(self);}, ms);
}
